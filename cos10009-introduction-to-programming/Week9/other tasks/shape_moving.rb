...
class GameWindow < Gosu::Window
  ...
  def update
    @shape_x += 4 if button_down?(Gosu::KbRight) && @shape_x < WIDTH - SHAPE_DIM
    @shape_x -= 4 if button_down?(Gosu::KbLeft) && @shape_x > 0
    @shape_y += 4 if button_down?(Gosu::KbDown) && @shape_y < HEIGHT - SHAPE_DIM
    @shape_y -= 4 if button_down?(Gosu::KbUp) && @shape_y > 0
  end
