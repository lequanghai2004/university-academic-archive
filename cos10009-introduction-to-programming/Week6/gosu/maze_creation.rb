...
class GameWindow < Gosu::Window
  ...
  def initialize
    ...
    while column_index < x_cell_count
      row_index = 0
      while row_index < y_cell_count
        @columns[column_index][row_index].north = @columns[column_index][row_index - 1] if row_index != 0
        @columns[column_index][row_index].south = @columns[column_index][row_index + 1] if row_index != y_cell_count - 1
        @columns[column_index][row_index].west = @columns[column_index - 1][row_index] if column_index != 0
  ...
  def print_cells
    ...
    while column_index < x_cell_count
      row_index = 0
      while row_index < y_cell_count
        north = @columns[column_index][row_index].north.nil? ? 0 : 1
        south = @columns[column_index][row_index].south.nil? ? 0 : 1
        east = @columns[column_index][row_index].east.nil? ? 0 : 1
        west = @columns[column_index][row_index].west.nil? ? 0 : 1
  ...
  def search(cell_x, cell_y)
    ...
    if cell_x == ((MAP_WIDTH / CELL_DIM) - 1)
      ...
      # A path was found:
      if !path.nil?
        puts 'Added x: ' + cell_x.to_s + ' y: ' + cell_y.to_s if ARGV.length > 0 # debug
        [[cell_x, cell_y]].concat(path)
      else
        puts 'Dead end x: ' + cell_x.to_s + ' y: ' + cell_y.to_s if ARGV.length > 0 # debug
        nil # dead end
      end
    end
  end
  ...
  # This is procedure i.e the return value is 'undefined'
  def draw
    print_cells if button_down?(Gosu::KbSpace)
    index = 0
    x_loc = 0
    y_loc = 0
    ...
    while column_index < x_cell_count
      ...
      while row_index < y_cell_count
        ...
        color = if @columns[column_index][row_index].vacant
                  Gosu::Color::YELLOW
                else
                  Gosu::Color::GREEN
                end
